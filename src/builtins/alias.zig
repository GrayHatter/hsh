...
pub fn call(_: *Hsh, titr: *ParsedIterator, a: Allocator, _: Io) Err!u8 {
    if (!std.mem.eql(u8, "alias", titr.first().resolved.str)) return Err.InvalidCommand;
    ...
    while (titr.next()) |t| {
        if (name) |_| {
            value = t.resolved.str;
        } else {
            if (std.mem.indexOf(u8, t.resolved.str, "=")) |i| {
                name = t.resolved.str[0..i];
                if (t.resolved.str.len > i + 1) {
                    value = t.resolved.str[i + 1 ..];
                    break;
                }
            } else {
                name = t.resolved.str;
...
fn del(src: []const u8, a: Allocator) Err!void {
    for (aliases.items, 0..) |al, i| {
        if (std.mem.eql(u8, src, al.name)) {
